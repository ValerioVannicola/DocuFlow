...
@click.option("--schema", "-s", required=True, help="Schema name or dotted Python path")
@click.option("--model", "-m", default="openai/gpt-4o", help="LLM model to use")
@click.option("--parser", "-p", default="pdfplumber", help="Parser: pdfplumber, tesseract, docling, smart, azure-di, textract, google-docai")
...
def serve(config_path: str, host: str, port: int) -> None:
    ...
    try:
        from docflow.serve import run_server
    except ImportError:
        click.echo(
            "Serve dependencies not installed. Run: pip install docflow[serve]",
            err=True,
        )
        sys.exit(1)

    click.echo(f"Starting DocFlow server on {host}:{port}")
...
def templates_list() -> None:
    ...
    from docflow.templates.registry import list_templates
...
def templates_init(name: str, target_dir: str | None) -> None:
    ...
    from docflow.templates.registry import _BUILTIN_DIR
    ...
    dest_dir = Path(target_dir) if target_dir else Path("docflow_templates")
