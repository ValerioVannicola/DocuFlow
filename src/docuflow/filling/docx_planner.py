...
def plan_docx_fill(
    ...
    *,
    ...
    if resolved == "content_controls":
        ...
        var_names = inspect_template_vars(path)
        docx_fields = [FormField(name=v, field_type="template_var") for v in var_names]
...
def _select_docx_strategy(path: str | Path, strategy: DocxFillStrategy) -> str:
    if strategy in ("content_controls", "template"):
        return strategy
    controls = inspect_content_controls(path)
    if controls:
        return "content_controls"
    if inspect_template_vars(path):
        return "template"
...
def _build_template_plan(
    *,
    ...
    data_fields = collect_data_fields(data, skip_none=skip_none)
    by_name = {v: FormField(name=v, field_type="template_var") for v in var_names}
    by_normalized = {normalize_key(v): by_name[v] for v in var_names}
    assignments: dict[str, Any] = {}
    filled: dict[str, FilledField] = {}
    warnings: list[str] = []
    errors: list[str] = []
    ...
    for df in data_fields:
        ...
        fv = format_value(df.value, format_spec=(formats or {}).get(df.name))
        assignments[form_field.name] = fv
        filled[df.name] = FilledField(
            field_name=df.name,
            value=df.value,
    ...
    assigned = set(assignments)
    unmapped_template = [v for v in var_names if v not in assigned]
    _apply_unmapped_policy(errors, warnings, unmapped_model, unmatched=unmatched, target_kind="DOCX template variable")

    if not var_names:
        errors.append("No {{ }} template variables were found in the document.")
    ...
    return FillPlan(
        ...
        pdf_fields=docx_fields,
        unmapped_model_fields=unmapped_model,
        unmapped_pdf_fields=unmapped_template,
        warnings=warnings,
        errors=errors,
    )
