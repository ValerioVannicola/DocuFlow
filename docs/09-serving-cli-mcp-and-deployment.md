...
The server name is `DocuFlow`. In BeaverMate, this is the document-intelligence layer. It exposes the following tools.

## MCP Agent Playbook

This is the guidance the LLM should follow when using the MCP surface.
...
- Use `extract_document` for normal text, native PDF, image, and OCR-based extraction. Use `extract_with_vision` when scans, tables, or layout make parser-based extraction unreliable.
...
- Use `fill_docx_form` for Word content-control forms. Use `get_pending_fills` -> `edit_fill_field` -> `approve_fill` / `reject_fill` for staged form writes.
- `store_path` defaults to `./.docuflow_store`. `document_id` links stored review/fill artifacts to follow-up actions.
- `correct_field` and `edit_fill_field` accept `new_value` as a string even when the underlying value is numeric or date-like.
